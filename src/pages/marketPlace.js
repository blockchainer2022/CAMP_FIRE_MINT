...
// import TestVideo from "../assets/images/test-video5.gif";
// import * as ReactBootstrap from 'react-bootstrap';
import Sample from "../assets/images/sampleNft.jpeg";
import TestVideo from "../assets/images/test.mp4";
...
function MarketPlaceGallary({
    ...
    if (!info.length) {
      nfts.forEach(async (nft) => {
        if (chainId === 97) {
          ...
          console.log("Data from URI", data);
          console.log("JSON DATA", data);
          const owner = await market_contract.methods
            .getNFTowner(contractAddress, data.edition)
            .call();
  ...
  const url = `https://defi.mobiwebsolutionz.com/api/nftmarketplace/get-nfts-list.php?contractAddress=${contractAddress}&filter=${
    select2 ? (select2 === "all" ? "all" : `owned&owner=${account}`) : "all"
  }&sort_by_price=${select ? select : "high-to-low"}`;
  ...
  return (
    <>
                        ...
                        <Link to={`/market-place-gallery/${val.tokenId}`}>
                          {val.tokenId > 7 ? (
                            <img
                              ...
                              src={val.image ? val.image : detail_Image}
                            />
                          ) : (
                            <video
                              autoPlay={true}
                              ...
                              style={{ width: "100%", height: 190 }}
                            >
                              <source src={val.image} type="video/mp4"></source>
                            </video>
                          )}
                        </Link>
                        <div class="contentP">
      ...
      <InformationModal
        ...
        onClose={setError}
        title="Oops"
        text="Trading will be open after Public sale is launched."
      />
    </>
  );
}
